	...
	*/
	...
	TextFormat.prototype.getImageData = function(s, tf)	// string, TextField - read only
	{
		...
		//for(var i=0; i<ubuff.length; i++) ubuff[i] = (imgd.data[4*i+3] << 24) | (imgd.data[4*i+2] << 16) | (imgd.data[4*i+1] << 8) | (imgd.data[4*i] );
		
		//console.log(imgd.data.length, ubuff.length);
		//for(var i=0; i<ui8buff.length; i+=4) if(ui8buff[i+3] == 0) ui8buff[i] = ui8buff[i+1] = ui8buff[i+2] = 255; 
		var ui8buff = new Uint8Array(imgd.data);
		var ubuff = new Uint32Array(ui8buff.buffer);	// another ArrayBufferView for the same buffer
